...
class ImageSelectorNode(ImageNodeBase):
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "hash_size": ("INT", {"default": 16, "min": 8, "max": 32}),
            }
        }
    ...
    def select_image(self, image_a, image_b, method, threshold, hash_size):
        # Calculate hashes based on selected method
        if method == "phash":
            hash_a = image_to_phash(image_a, hash_size=hash_size)
            hash_b = image_to_phash(image_b, hash_size=hash_size)
        elif method == "simhash":
            hash_a = image_to_simhash(image_a)
            hash_b = image_to_simhash(image_b)
        else:  # dhash
        ...
        distance = hamming_distance(hash_a, hash_b)
        should_update = distance > threshold
